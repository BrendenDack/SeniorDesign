...
try:
    from firmware.button_firmwareV2 import setup_button_firmware, Buttons, NavAction
    from firmware.battery_firmwareV2 import setup_battery_monitor
    from lib.LCD_2inch4 import LCD_2inch4
    from library import chat_libraryV4 as chat_library
...
def main_menu_callback(index, channel, bf):
    ...
    elif channel == Buttons.VOLUME_UP:
        VOLUME_LEVEL = min(100, VOLUME_LEVEL + 10)
        
    elif channel == Buttons.VOLUME_DOWN:
        VOLUME_LEVEL = max(0, VOLUME_LEVEL - 10)
    
    if prev_index != SELECTED_INDEX or channel in [Buttons.VOLUME_UP, Buttons.VOLUME_DOWN]:
        img = draw_gui(SELECTED_INDEX, VOLUME_LEVEL, BATTERY_INFO)
        update_display(img)
