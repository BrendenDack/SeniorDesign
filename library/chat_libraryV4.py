...
try:
    ...
    from firmware.button_firmwareV2 import Buttons, ButtonFirmware
...
def show_library(lcd, directory: Path, bf, battery_monitor, assets_path=None):
    ...
    def library_callback(index, channel, bf):
        try:
            ...
            if channel == Buttons.UP or channel == Buttons.DOWN:
            ...
            if prev_index != state["selected_index"] or channel in [Buttons.UP, Buttons.DOWN, Buttons.BACK, Buttons.SELECT]:
    ...
    try:
        bf = ButtonFirmware.get_instance(library_menu_items, state["selected_index"], library_callback)
        if not bf.is_active:
            bf.start()
